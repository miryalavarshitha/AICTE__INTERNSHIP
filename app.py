...
def chatbot_response(question):
    ...
    if "hello" in q or "hi" in q:
        ...
        return "Our garbage classification model achieved a training accuracy of 95.26% and a test accuracy of 94.14% using CNN and EfficientNetv2b2."
    elif "dataset" in q:
...
with gr.Blocks(css=css_code) as demo:
    ...
    <div class='about-section'>
        ...
        <p>This project focuses on automating garbage classification using Deep Learning techniques.
          A CNN with EfficientNet architecture achieved a training accuracy of 95.26% and a test accuracy of 94.14%.
    ...
    # Floating Chat Icon + JS for proper close
    gr.HTML("""
    <div class='floating-chat' onclick="document.querySelector('[id*=chatbox-container]').style.display='block';
    document.querySelector('[id*=chatbox-container] textarea').value='Have a great day!';
    document.querySelector('[id*=chatbox-container] button').click();">💬</div>
    <script>
    // Make sure Close button hides the chatbox
    document.addEventListener('click', function(e){
      if(e.target && e.target.innerText === 'Close'){
        document.querySelector('[id*=chatbox-container]').style.display='none';
      }
    });
    </script>
    """)
